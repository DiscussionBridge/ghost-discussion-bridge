...
export function nativePublication(record, config) {
  ...
  const provenance = `<hr><aside class="discussionbridge-publication"><p><strong>Published from <a href="${escape(topicUrl)}">The Bridge</a></strong></p><p>Source author: ${escape(authorName)} · Revision ${escape(source.revision)} · Ghost 6.59.0 · DiscussionBridge for Ghost 0.1.0-alpha.22</p></aside><div data-discussionbridge-comments="fullInteractive"></div><script src="/discussionbridge/assets/loader.js?v=0.1.0-alpha.22" defer></script>`;
...
export async function syncPublications(config, store, bridge, ghost) {
  ...
  await store.update(async (state) => {
    for (const record of candidates) {
      ...
      if (prior?.revision === publication.revision && prior?.canonical_url === publication.destination && prior?.adapter_version === "0.1.0-alpha.22") { summary.unchanged += 1; continue; }
      try {
        let result;
        if (prior) {
        ...
        state.publications[publication.resourceId] = { ghost_post_id: result.id, canonical_url: publication.destination, revision: publication.revision, adapter_version: "0.1.0-alpha.22", topic_id: publication.topicId, topic_url: publication.topicUrl, synchronized_at: new Date().toISOString() };
