...
export function renderOperatorPage(config, state, notice = "") {
  ...
  const summary = sync?.summary ?? { created: 0, updated: 0, unchanged: 0, skipped: 0, failed: 0, errors: [] };
  ...
  const failures = Array.isArray(summary.errors) && summary.errors.length
    ? `<section class="panel attention"><h2>Needs attention</h2><ul>${summary.errors.map((error) => `<li>${error.resource_id ? `<code>${escape(error.resource_id)}</code>: ` : ""}${escape(error.reason)}</li>`).join("")}</ul></section>` : "";
  ...
  </style></head><body><main class="wrap"><h1>DiscussionBridge for Ghost</h1><p class="lede">${escape(config.ghostOrigin)} · ${escape(PRODUCT_VERSION)}</p>${notice ? `<section class="panel notice">${escape(notice)}</section>` : ""}<div class="metrics">${metric("Ghost → Discourse mappings", posts.length)}${metric("Discourse → Ghost publications", publications.length)}${metric("Records needing attention", attention)}${metric("Last synchronization", syncState)}</div><section class="panel"><div class="actions"><div><h2>Publication synchronization</h2><p>${sync?.completed_at ? `Last completed ${escape(sync.completed_at)}.` : "No synchronization has been recorded."}</p></div><form method="post" action="/discussionbridge/operator/synchronize"><input type="hidden" name="csrf" value="${csrf}"><button type="submit">Synchronize publications</button></form></div><p>Created ${escape(summary.created ?? 0)} · Updated ${escape(summary.updated ?? 0)} · Already current ${escape(summary.unchanged ?? 0)} · Skipped ${escape(summary.skipped ?? 0)} · Failed ${escape(summary.failed ?? 0)}</p></section>${failures}<section class="panel"><h2>Mapped content</h2><div class="table-wrap"><table><thead><tr><th>Direction</th><th>Identity</th><th>Source</th><th>Destination</th><th>State</th><th>Last activity</th></tr></thead><tbody>${rows}</tbody></table></div></section></main></body></html>`;
}
