...
test("rich-content code injection is additive and idempotent", () => {
  ...
  const upgraded = mergeCodeInjection(script.replace("0.2.0-alpha.39", "0.1.0-alpha.99"));
...
test("maps an authoritative published Ghost post and its authors", async () => {
  ...
  assert.equal(record.adapter_version, "0.2.0-alpha.39");
...
test("Ghost source tag slugs accept Discourse case and normalize before internal tag construction", () => {
  const summary = forumSourceTopic({
    tags: [
      ...
      { id: 9, slug: "Subtitle-C\u2014Radiation-exposure", name: "Subtitle-C\u2014Radiation-exposure" },
    ],
  });
  ...
  assert.deepEqual(plan.sourceTagNames, [
    ...
    "#discussionbridge-source-subtitle-c-radiation-exposure",
  ]);
  ...
  const dashCollision = forumSourceTopic({ tags: [
    { id: 7, slug: "Subtitle-C-Radiation", name: "ASCII dash" },
    { id: 8, slug: "Subtitle-C\u2014Radiation", name: "Em dash" },
  ] });
  assert.throws(
    () => publicationPlan(dashCollision, { ...dashCollision, content_html: "<p>Duplicate dash taxonomy.</p>" }, { serverUrl: "https://forum.example" }),
    /Invalid source tag slug/u
  );
});
...
test("publication sync creates once, skips presentation records and exact retry is unchanged", async () => {
  ...
  assert.equal(state.publications[publicationRecord().resource_id].adapter_version, "0.2.0-alpha.39");
